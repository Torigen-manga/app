...
interface CategoryButtonWrapperProps {
...
function CategoryButtonWrapper({
	...
	manga,
	children,
}: CategoryButtonWrapperProps) {
	if (!isInLibrary) {
		return <MangaAddToLibrary manga={manga}>{children}</MangaAddToLibrary>;
	}
...
function CategoryHandler({ isInLibrary, manga }: CategoryHandlerProps) {
	return (
		<CategoryButtonWrapper isInLibrary={isInLibrary} manga={manga}>
			<Button
			...
			>
				<Bookmark
					className={cn("transition-colors", {
						"fill-primary-foreground": isInLibrary,
					})}
				/>
			</Button>
