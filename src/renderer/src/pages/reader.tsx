...
export default function Reader(): React.JSX.Element {
  ...
  const { data } = extensionMethods.useChapterDetails(source, mangaId, chapterId)
        ...
        if (bestIndex !== -1) {
          ...
          if (
            ...
            (bestIndex !== current &&
              ...
              bestRatio -
                (entries.find(
                  (e) => imageRefs.current.findIndex((img) => img === e.target) === current
                )?.intersectionRatio ?? 0) >
  ...
  return (
    <main className="relative flex w-full justify-center overflow-y-scroll">
      ...
      <div ref={mainRef} className="flex max-w-3xl flex-col">
