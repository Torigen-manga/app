...
import type { AppManga, APIResponse, AppLibrary, LibraryEntryTable } from '@common/index'
...
const useGetEntries = () =>
  useQuery({
    ...
    queryFn: async () => {
      const res: APIResponse<LibraryEntryTable[]> = await invoke(channels.library.getEntries)
...
const useHasEntry = (sourceId: string, mangaId: string) =>
  useQuery({
    ...
    queryFn: async () => {
      const res: APIResponse<boolean> = await invoke(channels.library.hasEntry, sourceId, mangaId)
...
function useAddMangaToLibrary() {
  ...
  return useMutation({
    ...
    mutationFn: async (data: AppManga) => {
      const res: APIResponse<void> = await invoke(channels.library.addEntry, data)
...
function useClearLibrary() {
  ...
  return useMutation({
    ...
    mutationFn: async () => {
      const res: APIResponse<void> = await invoke(channels.library.clearLibrary)
...
const useAddCategory = () => {
  ...
  return useMutation({
    ...
    mutationFn: async (category: string) => {
      const res: APIResponse<string> = await invoke(channels.category.addCategory, category)
...
function useRemoveCategory() {
  ...
  return useMutation({
    ...
    mutationFn: async (id: string) => {
      const res: APIResponse<void> = await invoke(channels.category.removeCategory, id)
...
function useAddCategoryToEntry() {
  ...
  return useMutation({
    ...
    mutationFn: async (data: { categoryId: string; libraryEntryId: string }) => {
      const res: APIResponse<void> = await invoke(
